...
namespace {
class TinyRISCVDAGToDAGISel final : public SelectionDAGISel {
...
void TinyRISCVDAGToDAGISel::PostprocessISelDAG() {
...
static SDNode *selectImm(SelectionDAG *CurDAG, const SDLoc &DL, int64_t Imm,
  ...
  TinyRISCVMatInt::generateInstSeq(Imm, XLenVT == MVT::i64, Seq);

  SDNode *Result = nullptr;
  SDValue SrcReg = CurDAG->getRegister(TinyRISCV::X0, XLenVT);
  for (TinyRISCVMatInt::Inst &Inst : Seq) {
    SDValue SDImm = CurDAG->getTargetConstant(Inst.Imm, DL, XLenVT);
    if (Inst.Opc == TinyRISCV::LUI)
      Result = CurDAG->getMachineNode(TinyRISCV::LUI, DL, XLenVT, SDImm);
    else
...
void TinyRISCVDAGToDAGISel::Select(SDNode *Node) {
  ...
  case ISD::FrameIndex: {
    ...
    ReplaceNode(Node,
                CurDAG->getMachineNode(TinyRISCV::ADDI, DL, VT, TFI, Imm));
    return;
  }
  case ISD::SRL: {
...
bool TinyRISCVDAGToDAGISel::SelectInlineAsmMemoryOperand(
...
bool TinyRISCVDAGToDAGISel::SelectAddrFI(SDValue Addr, SDValue &Base) {
...
void TinyRISCVDAGToDAGISel::doPeepholeLoadStoreADDI() {
  ...
  while (Position != CurDAG->allnodes_begin()) {
    ...
    if (!Base.isMachineOpcode() || Base.getMachineOpcode() != TinyRISCV::ADDI)
      continue;
...
// This pass converts a legalized DAG into a TinyRISCV-specific DAG, ready
