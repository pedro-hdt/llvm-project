//===----- TinyRISCVMergeBaseOffset.cpp - Optimise address calculations  ------===//
//
...
#define TinyRISCV_MERGE_BASE_OFFSET_NAME "TinyRISCV Merge Base Offset"
namespace {

struct TinyRISCVMergeBaseOffsetOpt : public MachineFunctionPass {
  static char ID;
  ...
  TinyRISCVMergeBaseOffsetOpt() : MachineFunctionPass(ID) {}
  ...
  StringRef getPassName() const override {
    return TinyRISCV_MERGE_BASE_OFFSET_NAME;
  }
...
char TinyRISCVMergeBaseOffsetOpt::ID = 0;
INITIALIZE_PASS(TinyRISCVMergeBaseOffsetOpt, "riscv-merge-base-offset",
                TinyRISCV_MERGE_BASE_OFFSET_NAME, false, false)
...
bool TinyRISCVMergeBaseOffsetOpt::detectLuiAddiGlobal(MachineInstr &HiLUI,
                                                  MachineInstr *&LoADDI) {
  if (HiLUI.getOpcode() != TinyRISCV::LUI ||
      HiLUI.getOperand(1).getTargetFlags() != TinyRISCVII::MO_HI ||
  ...
  if (LoADDI->getOpcode() != TinyRISCV::ADDI ||
      LoADDI->getOperand(2).getTargetFlags() != TinyRISCVII::MO_LO ||
...
// output from LoADDI.
void TinyRISCVMergeBaseOffsetOpt::foldOffset(MachineInstr &HiLUI,
                                         MachineInstr &LoADDI,
...
bool TinyRISCVMergeBaseOffsetOpt::matchLargeOffset(MachineInstr &TailAdd,
                                               Register GAReg,
                                               int64_t &Offset) {
  assert((TailAdd.getOpcode() == TinyRISCV::ADD) && "Expected ADD instruction!");
  ...
  if (OffsetTail.getOpcode() == TinyRISCV::ADDI) {
    ...
    if (AddiImmOp.getTargetFlags() != TinyRISCVII::MO_None)
      return false;
    ...
    if (OffsetLui.getOpcode() != TinyRISCV::LUI ||
        LuiImmOp.getTargetFlags() != TinyRISCVII::MO_None ||
    ...
    return true;
  } else if (OffsetTail.getOpcode() == TinyRISCV::LUI) {
...
bool TinyRISCVMergeBaseOffsetOpt::detectAndFoldOffset(MachineInstr &HiLUI,
                                                  MachineInstr &LoADDI) {
...
bool TinyRISCVMergeBaseOffsetOpt::runOnMachineFunction(MachineFunction &Fn) {
...
FunctionPass *llvm::createTinyRISCVMergeBaseOffsetOptPass() {
  return new TinyRISCVMergeBaseOffsetOpt();
}
