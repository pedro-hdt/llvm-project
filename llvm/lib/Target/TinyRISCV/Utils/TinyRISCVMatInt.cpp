//===- TinyRISCVMatInt.cpp - Immediate materialisation -------------*- C++ -*--===//
//
...
void generateInstSeq(int64_t Val, bool IsRV64, InstSeq &Res) {
  if (isInt<32>(Val)) {
    ...
    if (Hi20)
      Res.push_back(Inst(TinyRISCV::LUI, Hi20));

    if (Lo12 || Hi20 == 0) {
      unsigned AddiOpc = TinyRISCV::ADDI;
  ...
  Res.push_back(Inst(TinyRISCV::SLLI, ShiftAmount));
  if (Lo12)
    Res.push_back(Inst(TinyRISCV::ADDI, Lo12));
}
