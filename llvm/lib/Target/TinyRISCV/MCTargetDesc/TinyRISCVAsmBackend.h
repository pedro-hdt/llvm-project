//===-- TinyRISCVAsmBackend.h - TinyRISCV Assembler Backend -----------------------===//
//
...
public:
  TinyRISCVAsmBackend(const MCSubtargetInfo &STI, uint8_t OSABI, bool Is64Bit,
                  const MCTargetOptions &Options)
      : MCAsmBackend(support::little), STI(STI), OSABI(OSABI), Is64Bit(Is64Bit),
        TargetOptions(Options) {
    TargetABI = TinyRISCVABI::computeTargetABI(
        STI.getTargetTriple(), STI.getFeatureBits(), Options.getABIName());
    TinyRISCVFeatures::validate(STI.getTargetTriple(), STI.getFeatureBits());
  }
  ~TinyRISCVAsmBackend() override {}
  ...
  const MCFixupKindInfo &getFixupKindInfo(MCFixupKind Kind) const override {
    ...
    static_assert((array_lengthof(Infos)) == TinyRISCV::NumTargetFixupKinds,
